...
# 写一次性标记之前必须有**显式确认**：让用户知道接下来要点弹窗（用户 2026-09-20 的要求）。
if grep -q "准备好了吗" "$EXT_YES_LOG" \
   && grep -q "留在这台机器前" "$EXT_YES_LOG" \
   && grep -q "想要访问可移除宗卷上的文件" "$EXT_YES_LOG"; then
  pass "真机分支：写授权标记前有一道「准备点弹窗」的显式确认（含弹窗原文 + 留在机器前）"
else
  fail "真机分支：缺少「准备好了吗 / 留在机器前 / 弹窗原文」这道确认"
fi
if grep -q "磁盘管理 → 申请授权" "$EXT_YES_LOG"; then
  pass "真机分支：没确认时给出补救入口（面板「磁盘管理 → 申请授权」）"
else
  fail "真机分支：没有给出「磁盘管理 → 申请授权」这条补救路径"
fi
...
# 铁律 12：远程路径**一个字都不许改** —— 那道"准备好了吗"的确认只在真机分支问。
if grep -q "准备好了吗" "$EXT_REMOTE_LOG"; then
  fail "远程安装：竟然问了「准备好了吗」——远程点不了弹窗，绝不能出现这道确认"
else
  pass "远程安装：没有「准备好了吗」这道确认（铁律 12：远程绝不触发弹窗）"
fi
