...
func (s *Server) handleMirrorSync(w http.ResponseWriter, r *http.Request) {
	...
	s.launchTask(w, r, "mirror-sync", "mirror:"+dir, "同步镜像发布件到 "+dir, "mirror_sync",
		func(ctx context.Context, log tasks.LogFunc) (any, error) {
			return runMirrorSync(ctx, log, dir, base)
		})
}
...
// runMirrorSync 执行一次完整同步：验签清单 → 逐个下载核 sha256 → 全部通过后从
// 临时目录就位。任何一步失败都**不写镜像目录**（半截文件绝不外露）。幂等：重复
// 跑结果一致，同版本同 sha 的文件直接覆盖。
func runMirrorSync(ctx context.Context, log tasks.LogFunc, dir, base string) (*mirrorSyncResult, error) {
