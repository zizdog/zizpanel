...
//
// source 故意不传：空源时后端会走"配置里的源 → 镜像站 → 公网 → 镜像 → GitHub"
...
export function UpdateView(content, ctx = {}) {
      ...
      h('div.card-body', [
        h('pre.logbox', {
          text: [
            ...
            '',
            '# 卸载面板（三档；逐行显示正在卸载/删除什么）',
            'sudo /opt/zizpanel/uninstall.sh                 # 交互菜单选 1/2/3',
            'sudo /opt/zizpanel/uninstall.sh --mode 1        # 仅卸载面板（保留数据与环境）',
            'sudo /opt/zizpanel/uninstall.sh --mode 2        # 面板 + LNMP（保留 ~/www 与数据库数据）',
            'sudo /opt/zizpanel/uninstall.sh --mode 3        # 面板 + 所有环境与数据',
            'sudo /opt/zizpanel/uninstall.sh --dry-run 3     # 预演，一个字节都不改',
            '# ⚠️ 模式 3 不可逆：会删数据库数据与面板装过的软件',
          ].join('\n'),
        }),
      ]),
    ]),
  ...
  function render(next) {
    ...
    // ---- 操作按钮 ----
    //
    // 这里**不再**放"立即检测 + 一键更新"两颗按钮（用户明确要求删掉那一段：
  ...
  async function oneClickUpdate() {
    ...
    try {
      ...
      if (!staged) {
        ...
        // 用户因此能看到进度与已用时长，而不是盯着一句 toast 干等（用户要求）。
        stageInflight = true;
