...
//  写死清华源；现在改成与其它来源一致的"NAS 优先、探不通回落清华"
//  （政策见 mirror.go 文件头）。
//
//  NAS 侧怎么做到**轮子也走镜像**（2026-09-16 实测确认，别再改成改写 HTML）：
//    清华 TUNA 的 simple 索引里文件链接是**相对路径** ../../packages/…，
//    pip 用 urljoin 按"请求索引页时用的基址"解析，于是
//      <base>/pypi/simple/<pkg>/   →   <base>/pypi/packages/…
//    刚好落回镜像自身。所以 NAS 的 nginx 只要有 /pypi/simple/ 与
//    /pypi/packages/ 两条 location 就够了（见 NAS 上 zizpanel-mirror 的 nginx.conf）。
...
//    NAS 侧 8091 的按需缓存**没有 TTL**，索引页一旦落盘就不再回源。上游发了
//    新版本后，镜像上要手动清一次：
...
// pipMirrorPath 是 PyPI 索引在镜像上的子路径（NAS 侧 nginx 的 location 前缀）。
...
//     NAS 侧 8091 的按需缓存是**整个文件落盘后**才回第一个字节的，冷缓存拉大
...
//  3. 离线模式（仅走 NAS）下探不通 → **明确失败、不回落**（见 MirrorOfflineOnly）。
//
	...
	if _, err := m.probeMirrorFile(ctx, probe); err != nil {
		...
		if result != nil {
			result.step(ctx, fmt.Sprintf(
				"NAS 上没有 PyPI 索引（%v），pip 改用清华源：%s", err, pipIndexFallback))
		}
		return []string{"-i", pipIndexFallback}, nil
	}
	if result != nil {
		result.step(ctx, "pip 索引：NAS 镜像 "+indexURL+"（已探通；第二次安装直接命中 NAS 缓存）")
	}
	args := []string{"-i", indexURL, "--timeout", "180", "--retries", "3"}
	// 镜像基址是 http（例如局域网 http://192.168.1.8:8090）时 pip 默认拒绝，
