...
77. **装完的应用没有卸载入口** → 用户反馈：应用市场里装上 IOPaint 之后，
    面板里**没有任何办法把它卸掉**。根因是"能装 ≠ 能卸"这件事没被当成一回事：
    面板的通用卸载（`Manager.Uninstall`）只认 `managed=1` 的记录，而面板自研安装器
    注册服务时用的是 `Adopted`（纳管）语义 —— 于是一点卸载就报
    "这是纳管服务（由你自己安装），面板不会卸载它"，而这台机器上它明明是面板装的。
    修法：按**来源**分三类给不同入口（托管服务 / 面板安装器 / 纳管第三方），
    并且卸载前先把"会执行哪些步骤、会删哪些路径、会保留什么"列进确认框。
    教训：**"纳管"这个词被当成了"用户装的"的同义词**，但面板自己的安装器也在用它 ——
    判断"能不能卸载"要看 `PanelInstaller`，不能只看 `managed`。

78. **"验证更新阻断"这个动作在任何机器上都失败，报的话还是错的** →
    `VerifyUpdatesBlocked` 里写死了 `/usr/bin/softwareupdate`，
    而 macOS 上它在 **`/usr/sbin`**（两台 Mac 实测确认）。
    后果有两层，第二层更有害：动作必然失败；而失败信息是
    "仍能查到更新（阻断可能没生效）" —— 把"命令根本没跑起来"说成了"阻断失效"，
    用户会去反复重做阻断，永远修不好。
    修法：按候选路径（`/usr/sbin` → `/usr/bin` → `PATH`）解析，找不到时
    明确报"找不到 softwareupdate 命令"，并有一条真机断言锁住这个路径。
    教训：**把"工具没跑起来"和"结论是否定"分开报** —— 这两者的处置完全不同。

79. **判定把"命令没跑起来/退出码非 0"当成"结论是否定"** →
    修好路径之后又暴露一层：`softwareupdate --list` 在 hosts 阻断生效时会打印
    "错误的URL"，但**退出码是 0**。而判定把"看到错误文本"绑在 `err != nil` 上，
    于是"阻断明明有效"被判成"仍能查到更新（阻断可能没生效）" ——
    比不报更糟：用户会去反复重做阻断，永远修不好。
    修法：抽出纯函数 `judgeSoftwareupdate`，判定顺序改为
    ①输出里有"更新目录不可达"的明确特征 → 已阻断（**不看退出码**）；
    ②`no new software` → 也没有可用更新；③命令失败但输出不像上面两种 →
    如实说"无法据此判断"（这**不是**"阻断失效"）；④其余（真的列出更新）→ 没阻断。
    特征表刻意收窄（不用宽泛的"错误"/"could not"），否则网络断了、证书坏了
    都会被说成"阻断有效"。单测直接用两台真机抓下来的原始输出。

80. **回滚把用户原本的选择改掉了** → `RestoreUpdates` 早期一律把 5 个偏好写成
    `true`（"系统默认是开的"），但它们在阻断之前未必都是 `true` ——
    真机上"恢复系统更新"就顺手改掉了用户的选择。
    修法：阻断时把**改动前的原值**记进 hosts 阻断块里的一行注释
    （`# zizpanel-prev-prefs: AutomaticDownload=0 …`），回滚按它逐个恢复；
    键原本不存在就记为 `unset`，回滚时不凭空写值。
    记在 hosts 块里的好处：摘块时记录自动消失，不留孤儿状态文件。
    顺带一提，这个函数第一版还有个低级的 Go 错：`for _, k := range someMap`
    拿到的 k 是**值**不是键，拼出来一堆 `=0` —— 单测当场抓到。

81. **配了加速源还是慢：docker 只用「第一个能应答」的源** →
    第一版把用户勾选的加速源按**字母序**写进配置，于是 `docker.1ms.run` 排在第一个。
    而 dockerd 的行为是：**只用第一个能应答的镜像源**，不会因为某个源慢就自动
    换下一个（真机日志里能看到它一直在用那一个：`msg="Host doesn't match"
    cfgHost=registry-1.docker.io host=docker.1ms.run`）。
    结果就是"配了加速源，速度全看运气"。
    修法：保存前**先实测一遍**，按（可用优先、延迟升序）排序再写入，
    并把实测结果打进任务日志 —— 用户能看到为什么是这个顺序。
    教训：这类"顺序有语义"的配置，别用 `sort.Strings` 图省事。

82. **多站点共用一个音色样本：后传的覆盖先传的，还说不出是谁的** →
    接收端 v1.4.0 把**所有**网站上传的参考音频都写进同一个
    `~/tts/voice-samples/ref.wav`，而且只按请求头里的文件名判断类型：
    一个 mp3 被命名成 `ref.wav` 就直接落盘。上游 mlx-audio **按扩展名选解码器**，
    解不开时返回的是 **HTTP 200 + 0 字节** —— 网站侧只看到
    `IncompleteRead(0 bytes read)`，从错误信息完全推不到"样本文件是假的 wav"，
    更难想到"这份样本其实是另一个站上传的"。真机上那份 `ref.wav`
    就是被覆盖后的产物，ffprobe 都解不开。
    修法（v1.5.0）：样本按来源存 `<目录>/<来源>/ref.wav`（来源标识洗成
    `[A-Za-z0-9_-]`，**不复用文件名那套 `safe_name()`**，否则 `..` 会进目录名）；
    上传先按魔数+ffprobe 校验内容，解不开直接 400 并说清原因；再统一归一化成
    ≤12 秒 / 24kHz / 单声道 / PCM s16le wav 后原子替换；`/jobs` 带 `source`
    并在 `GET /jobs/{id}` 回传，取样本顺序是
    `显式 ref_audio → <目录>/<source>/ref.wav`（**没有旧版回退**，
    根目录那份 v1.4.0 遗留文件不参与解析 —— 插件仍在开发期，与其猜"该用哪份音色"，
    不如明确报错）。没有 ffmpeg 时不假装成功，直接说清只能收 24kHz wav。
    教训：**"共享一份可变文件"的接口，迟早会把两个用者的状态搅在一起**；
    而"上游 200 但 0 字节"这种失败模式必须在接收端就拦掉。
...
| 加速源 | **换源**：看 Docker Hub 通不通、检测内置加速源、一键写入并重启（见下） |
...
> 起因是用户一句"现在的拉取太慢了，没法用"。实测确认：这台机器
> **直连 `registry-1.docker.io` 会超时**（拉 alpine 卡 30 秒后报
> `dial tcp 199.96.61.1:443: i/o timeout`），所以不配加速源根本拉不动。

「加速源」分区做四件事：

1. **先把官方源通不通摆出来**（页面最上方一条横幅）—— 用户抱怨的"慢"就是它，
   先确认问题出在哪，而不是让人怀疑自己网络。
2. **内置候选 + 实测结论**：13 个常用加速源，每条都写清实测结果
   （ustc / 163 / 百度 / kubesre / dockerpull **实测已超时**、南大 403、
   dockermirror TLS 失败），不让用户去点一个死链接。
3. **检测可用性**：并发探测 `<url>/v2/` 并显示延迟。判定按**真实回包**——
   `200` 与 **`401` 都算可用**（401 是 registry 要求鉴权，是它正常工作的证据）；
   `403`=被拒绝、`302`=跳转、超时=连不上。
4. **保存并重启**：写入 `~/.colima/<profile>/colima.yaml` 的 `docker:` 段
   （先备份，其它内容一字不动）→ `colima restart` → 复核 `docker info` 真的读到了
   → 再**实测拉一个小镜像**并把耗时写进日志。走任务中心（重启约 30-60 秒，
   期间容器会短暂中断；compose 应用有 `restart` 策略，起来后会自动恢复）。

只对**能验证**的运行时动手：Colima 自动配置；OrbStack / Docker Desktop 如实告知
面板不改写它的配置（宁可不动，也不写一个没验证过的文件）。

实测效果（两台机器，配 `daocloud + 1ms.run + 1panel.live` 之前/之后）：

| | 配之前 | 配之后 |
|---|---|---|
| 本机拉 `alpine:3.20` | **失败**：30 秒后 `dial tcp i/o timeout` | 成功（首次 54s，镜像站缓存后重拉 10s） |
| mini 拉 `alpine:3.20` | **失败**：同上 | 成功（首次 13s，重拉 6-18s） |
| mini 的容器 | — | colima 重启后 `it-tools` 与 `uptime-kuma` 都自动恢复（healthy） |

**速度取决于镜像站有没有缓存过这个镜像**：没缓存时是镜像站替你去 Docker Hub 取
（第一次慢），取过一次之后明显快。想更快就换一个源再测 —— 工具就在这一页。
...
**v1.5.0：音色按来源隔离**（见 `HANDOFF-TO-PANEL-1.7B.md` §3.5）。

以前所有网站上传的样本都写进**同一个** `~/tts/voice-samples/ref.wav`：
后传的覆盖先传的，两个站用一个接收端时，用户听到的是**别的站的音色**，
而界面上完全看不出来。更糟的是上传只按**请求头里的文件名**判断，
一个 mp3 被命名成 `ref.wav` 就直接落盘；上游 mlx-audio 按**扩展名**选解码器，
解不开时它返回的是 **HTTP 200 + 0 字节** —— 网站只看到
`IncompleteRead(0 bytes read)`，看不出跟"样本文件是假的 wav"有关。

现在：

1. **按来源落盘**：`POST /voice` 多一个 `X-TtsVoice-Source` 头（缺省 `default`），
   样本存到 `<样本目录>/<来源>/ref.wav`。来源标识会被洗成
   `[A-Za-z0-9_-]`（其余换 `-`、首尾去掉、截 64 字符，空则 `default`），
   **刻意不复用文件名的 `safe_name()`** —— 那个会保留点号，落到目录名上就是 `..`。
2. **按内容校验**：先看魔数（明显不是音频的直接拒），再用 ffprobe 真解码一次；
   解不开就 **400** 并说明原因。然后归一化成 **≤12 秒 / 24kHz / 单声道 / PCM s16le WAV**
   （原子替换，不留下半个文件）。**不足 1 秒直接 400**（太短的样本克隆出来的音色不像本人），
   1–3 秒返回 200 但带 `warning`，超过 12 秒自动截断并标 `truncated`。
   响应：`{ok,path,name,source,format,duration,size,sha256}`。
   没有 ffmpeg 时**不假装成功**：只放行本来就是 24kHz/16bit/单声道的 wav，其余明确报错。
3. **`/jobs` 带 `source`**：取参考音频的顺序是
   `显式 ref_audio` → `<目录>/<source>/ref.wav`，**没有旧版回退** ——
   根目录那份 v1.4.0 遗留的 `ref.wav` 不参与解析，具名来源没有样本就报
   `voice sample not found`。宁可明确失败，也不猜"该用哪份音色"
   （插件仍在开发期，不存在必须兼容的老客户端）。
   `source` 会记进作业、写进日志，并在 `GET /jobs/{id}` 里返回。
4. **面板侧「各来源」**：「服务管理 → TtsVoice 音色接收端 → 详情 → 🎙 各来源」
   列出每个来源（标识 / 时长 / 大小 / sha256 前 8 位 / **最后使用时间**），
   可以**单独替换**（同名来源再传一次即替换）或**删除**一个来源，
   也可以先手动「上传新来源」。v1.4.0 遗留在根目录的 `ref.wav` 也会列出来
   （标成"残留，已不再使用"），方便在界面上直接删掉；
   时长读不出来的（例如被改名的 mp3）显示为「⚠️ 无法解析」。
   列表与替换都走接收端的 `/voice/sources`、`/voice`：**解析与转码只有一处实现**，
   面板不重复发明"什么算合法音频"。

> 这一版**必须重新部署一次接收端**才会生效：`receiver.py` 是随面板二进制内嵌的，
> 升级面板本身不会重写已经在跑的那份脚本。重新部署会**沿用原有监听地址**
...
原来只有一把 plist 里的 `--token`：**任何一处泄露都要全员换**，分不清谁在用，
也没法限制单个站点用多少。现在：

1. **多密钥**：「服务管理 → TtsVoice 音色接收端 → 详情 → 🔑 调用密钥 → ➕ 添加密钥」，
   每个网站（或每个调用方）一把。入口就是**添加**，不是"更改共享密钥"。
   密钥值留空自动生成 `ttsv-<32 hex>`，也可以填已有的值（方便把现有网站接过来）。
   新增时会把完整密钥显示一次（可复制），之后列表里只给掩码。
2. **热加载**：密钥与额度存在 `~/tts/voice-receiver/keys.json`，接收端**按 mtime 热加载** ——
   加/停/删/改额度**都不需要重启**（它可能正在替用户合成），传进网站插件后**立刻生效**。
   plist 里不再写明文密钥，只告诉接收端这个文件在哪。
3. **每密钥额度（字）**：按**实际合成完成**的字数计（`len(text)`，中文 1 字算 1、含标点），
   `0` = 不限。
   - 提交时先**占用**额度（防止同一把密钥并发提交把总额度撑爆；剩余的显示里也扣掉了占用）；
   - 作业到终态（完成 / 失败 / 取消）时按**已经合成完的块**结算，
     没跑到的部分自动退回 —— **生成失败或中途取消不按整篇收费**，这是 v1.7.0 改的口径
     （以前是提交时按整篇扣，等于把"我们的失败"算在用户头上）；
   - 直连 `/v1/audio/speech` 只在响应 2xx 时计费；
   - 额度用完返回 **429**，错误串以 `quota exceeded:` 开头 —— 刻意与"密钥无效"的 **403**
     区分开，否则调用方会把"额度用完"当成"密钥错了"去重试；
   - 幂等重放（同一 `client_id`、任务还在队列里）**不重复占用**。
4. **用量统计**：`GET /usage` 按密钥累计 `chars / requests / jobs / audio_bytes`，
   另存最近 60 天的按天明细。面板里一眼看到：今日 / 近 7 天 / 累计字数、调用次数、
   产出音频大小、14 天趋势条，以及**每把密钥**的额度、已用、剩余（带进度条）、今日、最后使用。
5. **手动清零**：每把密钥行内有「清零」（顶部还有「🧹 清零全部用量」）—— 把已用字数
   与调用次数归零、重新开始计，**密钥与额度都不变**。清零是**管理动作**：接收端要求
   `X-TtsVoice-Admin`（管理密钥 `--admin-token` 在部署时生成、写进 plist，只有面板拿得到）。
   刻意不让普通调用密钥清零自己的额度 —— 否则额度只是建议。
   清零时**正在跑的作业**不受影响，它们完成后仍按实际完成的字数记进来。
6. **不会被自己锁死**：不允许停用或删除**最后一条启用的密钥**（那等于一键让所有网站 401）；
   删除密钥只是撤销访问，历史用量仍留在统计里（所以"总量"永远对得上）。
   调用被拒（密钥错/停用）不计用量，上游失败只计一次请求、不计字数与字节。

> 迁移：老机器上 plist 里的那把 `--token` 会在"重新部署接收端"时原样搬进
> `keys.json` 成为 `default` 密钥（网站不用改），plist 随即不再写密钥。
> 迁移完成前，面板的密钥列表会把那把兼容密钥显示成「来自服务配置，未迁移」并只读。
...
已安装的应用卡片上有卸载入口，但它做的事**按来源分三类**（`services.UninstallPlan`）：

| 来源 | 入口 | 做什么 |
|---|---|---|
| 面板托管服务（compose 应用等，`managed=1`） | **卸载** | 停服务 + 删定义 + 删记录（compose 只删容器与网络，**具名卷保留**） |
| 面板自研安装器装的（IOPaint / Qwen / 接收端 / phpMyAdmin / Docker 运行时） | **卸载** | 卸服务 + 删 plist + 删记录；虚拟环境、模型、音色样本、历史任务默认**保留**，勾选「同时删除数据」才删 |
| 纳管的第三方服务（nginx / php / mysql / 你自己注册的） | **取消纳管** | 只把记录从面板移除，**不跑 brew uninstall、不删任何文件** |

第三条是铁律：删掉用户自己装的 MySQL 等于删掉他的网站数据。
所以市场对这类应用**不提供卸载**，并把原因写在确认框里。

两个容易踩的细节（都有测试）：
- phpMyAdmin 的卸载必须**先撤 nginx 入口、再 brew uninstall**：反过来的话
  会留下一段指向不存在目录的 location（访问 404/502，看起来像面板把站点弄坏了）；
  撤入口按安装器写的注释锚点配对花括号，不会误删用户自己的 location。
- Docker 运行时在还有 compose 应用登记时**拒绝卸载**（否则那些应用会变成孤儿），
  界面与后端都会拦（不能只靠界面）。
