...
3. **每密钥额度（字）**：按**提交文本的字符数**计费（`len(text)`，中文 1 字算 1、含标点）。
   `0` = 不限。额度用完时接收端返回 **429**，错误串以 `quota exceeded:` 开头 ——
   刻意与"密钥无效"的 **403** 区分开，否则调用方会把"额度用完"当成"密钥错了"去重试。
   幂等重放（同一 `client_id`、任务还在队列里）**不重复扣**。
...
5. **不会被自己锁死**：不允许停用或删除**最后一条启用的密钥**（那等于一键让所有网站 401）；
