...
82. **多站点共用一个音色样本：后传的覆盖先传的，还说不出是谁的** →
    ...
    `显式 ref_audio → <目录>/<source>/ref.wav → <目录>/ref.wav`，
    最后那份**只给 `default` 兜底**（具名来源没样本就明确报错，
    绝不悄悄用别的站的音色）。没有 ffmpeg 时不假装成功，直接说清只能收 24kHz wav。
...
> （本机 127.0.0.1 不会被悄悄改成 0.0.0.0）并复用原密钥。
